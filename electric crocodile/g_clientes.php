...
<body>
	...
	<section>
		...
		//MUESTREO DE CLIENTE
		$instruccion_cliente="SELECT * FROM cliente";
		$consulta_cliente=mysqli_query($connect,$instruccion_cliente);
		if($consulta_cliente){
			/*echo "SE REALIZO LA CONSULTA DE LA TABLA CLIENTES";*/
			$validacion=1;
		}else{
		...
		if($validacion==1){
				...
				echo '<tr>
					...
					<td id='tdgc'>".$fila['nombre_cliente'] . " "."</td>
					<td id='tdgc'>".$fila['usuario'] . " "."</td>
					<td id='tdgc'>".$fila['telefono'] . " "."</td>
					<td id='tdgc'>".$fila['direccion'] . " "."</td>
					<td id='tdgc'>".$fila['ciudad'] . " "."</td>
					<td id='eliminar_td'>
						<a href='eliminar_cliente.php?curp=".$fila['curp']."'>Eliminar</a>
					</td>
					<td id='editar_td'>
						<a href='editar_cliente.php?curp=".$fila['curp']."'>Editar</a>
					</td>
				</tr>";
				}
